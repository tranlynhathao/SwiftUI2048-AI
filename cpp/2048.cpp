...
int main(int argc, char *argv[]) {
  ...
  for (int game = 1; game <= iterations; ++game) {
    ...
    for (;;) {
      ...
      if (detailed) {
        std::cout << "Progress:" << std::setw(6) << (1 << maxTile) << ' ' << Progress(board) << ' '
                  << (1 << (maxTile + 1)) << '\r';
        std::cout.flush();
      }
    }
  ...
  fout << "Average" << ',' << sumScore / iterations << ',' << sumMoves / iterations << ',' << sumTime / iterations
       << ',' << sumSpeed / iterations;
}
